/*
===============================================================================================
Stored Procedure: Load Bronze Layer (Truncate & Insert) - Parametric Version
===============================================================================================
Script Purpose:
...
Parameters:
    p_data_source_path (TEXT): The absolute base path to the 'datasets' folder.

Usage:
    CALL bronze.load_bronze_layer('/absolute/path/to/datasets');
===============================================================================================
*/

CREATE OR REPLACE PROCEDURE bronze.load_bronze_layer(p_data_source_path TEXT)
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    -- ------------------------------------------------------------------------------------------
    -- Table: crm_cust_info
    -- Action: Truncate and Load from CSV
    -- ------------------------------------------------------------------------------------------
    table_start_time := clock_timestamp();
    RAISE NOTICE '>> Loading table: bronze.crm_cust_info';

    TRUNCATE TABLE bronze.crm_cust_info;
    EXECUTE format('COPY bronze.crm_cust_info FROM %L WITH (FORMAT CSV, HEADER, DELIMITER '','');', p_data_source_path || '/source_crm/cust_info.csv');
    ...
    EXECUTE format('COPY bronze.crm_prd_info FROM %L WITH (FORMAT CSV, HEADER, DELIMITER '','');', p_data_source_path || '/source_crm/prd_info.csv');
    ...
    EXECUTE format('COPY bronze.crm_sales_details FROM %L WITH (FORMAT CSV, HEADER, DELIMITER '','');', p_data_source_path || '/source_crm/sales_details.csv');
    ...
    EXECUTE format('COPY bronze.erp_cust_az12 FROM %L WITH (FORMAT CSV, HEADER, DELIMITER '','');', p_data_source_path || '/source_erp/cust_az12.csv');
    ...
    EXECUTE format('COPY bronze.erp_loc_a101 FROM %L WITH (FORMAT CSV, HEADER, DELIMITER '','');', p_data_source_path || '/source_erp/loc_a101.csv');
    ...
    EXECUTE format('COPY bronze.erp_px_cat_g1v2 FROM %L WITH (FORMAT CSV, HEADER, DELIMITER '','');', p_data_source_path || '/source_erp/px_cat_g1v2.csv');
